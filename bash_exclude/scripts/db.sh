...
--data "$DEFAULTDATA" "$FORMACTION" -s)

DEPARTURES=$(echo "$RESPONSE" | grep -zoP '(?<=<td class="time" style="min-width:130px;">)(\n)?[0-9]+:[0-9]+' | tr -d '\0' | tr '\n' ' ')
ARRIVALS=$(echo "$RESPONSE" | grep -zoP '(?<=<td class="time">)(\n)?[0-9]+:[0-9]+' | tr -d '\0' | tr '\n' ' ')
DURATIONS=$(echo "$RESPONSE" | grep -zoP '(?<=<td class="duration lastrow" rowspan="2">)(\n)?[0-9]:[0-9]+' | tr -d '\0' | tr '\n' ' ')
PRODUCTS=$(echo "$RESPONSE" | grep -zoP '(?<=<td class="products lastrow" rowspan="2">)(\n)?[A-Z]+' | tr -d '\0' | tr '\n' ' ') 
read -rd '' -a dep <<< "$DEPARTURES"
read -rd '' -a arr <<< "$ARRIVALS"
read -rd '' -a dur <<< "$DURATIONS"
read -rd '' -a pro <<< "$PRODUCTS"
for i in "${!dep[@]}"; do
  echo "${dep[$i]} ${arr[$i]} ${dur[$i]} ${pro[$i]}"
done | column -c 1
